...
let uploadDocs = require("../../upload").uploadDocs;
...
router.get('/', async function(req, res) {
  res.render('timetable.hbs', {
    timetable: await timetableRepository.getAll(),
    programs: await programsRepository.getAll(),
    page: "timetable",
    timetableDoc: await timetableRepository.getByTitle("timetableDoc"),
    admin: res.admin});
});
...
router.post('/add/photo', uploadDocs.single("timetable"), async function (req, res) {
  let doc = await timetableRepository.getByTitle("timetableDoc");
  doc.src = "/docs/" + req.file.filename;
  timetableRepository.update({"title": "timetableDoc"}, doc).then(
    res.redirect("/timetable")
  );
})

...
router.get('/', async function (req, res) {
  res.render('teachers.hbs', {teachers: await teacherRepository.getAll(), page: "teacher"});
});

router.post('/add', uploadImage.array("image", 1), async function (req, res) {
  let model = {};
...
router.get('/delete/:name', async function (req, res) {
  teacherRepository.delete({fullName: req.params["name"]}).then(r => {
    res.redirect("/teachers")
  })
});
